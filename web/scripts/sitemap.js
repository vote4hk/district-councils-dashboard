...
<url><loc>https://vote4.hk/</loc><lastmod>${moment().format(
...
async function main() {
  ...
  const peopleLinks = dcd_people.map(
    person =>
      `<url><loc>https://vote4.hk/profile/${person.name_zh || person.zame_en}/${
        person.uuid
  ...
  const districtLinks = dcd_constituencies.map(
    c =>
      `<url><loc>https://vote4.hk/district/2019/${c.code}</loc><lastmod>${lastMod}</lastmod><changefreq>daily</changefreq><priority>1.00</priority></url>`
  )
