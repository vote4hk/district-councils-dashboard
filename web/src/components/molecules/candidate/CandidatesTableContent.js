...
class CandidatesTableContent extends Component {
  matchCamp(candidate, showDemocracy, showEstablishment, showBlank, showOthers) {
    if (showDemocracy && candidate.camp == "民主")
      return true
    if (showEstablishment && candidate.camp == "建制")
      return true
    if (showOthers && candidate.camp == "其他")
      return true
    if (showBlank && candidate.camp == "")
      return true
    return false
  }
  ...
  render() {
    ...
    const { candidates, showEstablishment, showDemocracy, showBlank, showOthers } = props
    return (
      <>
        {candidates.filter(candidate => matchCamp(candidate, showDemocracy, showEstablishment, showBlank, showOthers)).map(candidate => (
          <StyledTableRow
