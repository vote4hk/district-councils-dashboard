...
const addElection = (obj, cur) => {
  ...
  delete obj.gender
  if (
    cur.estimated_birth !== obj.estimated_birth &&
    !cur.estimated_birth.includes('/')
  ) {
    cur.estimated_birth = `${Math.min(
      parseInt(cur.estimated_birth),
      parseInt(obj.estimated_birth)
    )}/${Math.max(
      parseInt(cur.estimated_birth),
      parseInt(obj.estimated_birth)
    )}`
...
csv()
  ...
  .then(jsonObj => {
    jsonObj.forEach(obj => {
      // Existing People
      const cur = output.find(
        o =>
          o.name_chi === obj.name_chi &&
          Math.abs(
            parseInt(o.estimated_birth) - parseInt(obj.estimated_birth)
          ) <= 1
      )
      if (cur) {
