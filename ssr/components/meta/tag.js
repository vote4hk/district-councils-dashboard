...
const TagMeta = (props) => {
  const { lang, year, tag } = props

  const meta = lang === 'en' ? en : zh

  const metaCanonicalUrl = meta.formatTagCanonicalUrl(year, tag)
  const metaSiteMap = meta.formatSiteName()
  const metaTitle = meta.formatTagTitle(tag)
  const metaDescription = meta.formatTagDescription(tag)
